...
function CommissionDetails() {
    ...
    const handleButtonClick = async () => {
        try {
            const address = `http://127.0.0.1:8086/cancelCommission/${commissionId}`;
            const response = await fetch(address, {
                method: "POST",
                headers: {
                    'Content-Type' : 'application/json', 
            ...
            if(response.ok) {
                alert("Pomyślnie zmieniono status paczki");
                window.location.reload();
            } else {
                console.error("CommissionDetails: ", response.statusText);
				alert("Wystąpił błąd");
            }
        } catch (error){
            console.error(error);
        ...
        <div className="site-first-div" style={{ marginLeft }}>
            ...
            {loading ? (
                ...
                <div className="commissionhistory-table-container">
                    ...
                    <br />
                    <button className='deactivateaccount-submit' onClick={handleButtonClick} disabled={loading}> Zmień status </button>
                </div>
                </div>
            )}
        </div>
